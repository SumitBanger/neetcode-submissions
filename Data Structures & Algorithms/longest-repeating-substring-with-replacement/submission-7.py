    ...
    def characterReplacement(self, s: str, k: int) -> int:
        ...
        for index, char in enumerate(s):
            ...
            if currentWindowLength - maxCharCount <= k:
                print(f"inside if, currentWindowLength: {currentWindowLength}, maxCharCount: {maxCharCount}, index: {index}, left: {left}")
                maxLength = max(maxLength, currentWindowLength)
            else:
                print(f"inside else, currentWindowLength: {currentWindowLength}, maxCharCount: {maxCharCount}, index: {index}, left: {left}")
