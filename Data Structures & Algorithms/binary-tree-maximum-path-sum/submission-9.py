...
class Solution:
    ...
    def dfs(self, root: Optional[TreeNode], result):
        ...
        nodeVal = root.val
        maxWithNodeAndChildren = nodeVal + max(leftMax, 0) + max(rightMax, 0)
        maxWithNodeAndOneChild = nodeVal + max(max(leftMax, 0), max(rightMax, 0))
        maxWithoutNode = max(leftMax, rightMax)
        maxTillCurrentNode = max(nodeVal, maxWithNodeAndChildren, maxWithNodeAndOneChild, maxWithoutNode)
