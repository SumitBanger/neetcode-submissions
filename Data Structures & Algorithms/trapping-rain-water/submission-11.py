class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        total = len(height)
        maxTillNowFromFront, maxTillNowFromBack, waterStored, maxTillNow = [0], [0], 0, 0
        for index in range(1, total):
            maxTillNow = max(maxTillNow, height[index - 1])
            maxTillNowFromFront.append(maxTillNow)

        maxTillNow = 0
        for index in range(total - 2, -1, -1):
            maxTillNow = max(maxTillNow, height[index + 1])
            maxTillNowFromBack.insert(0, maxTillNow)
        
        for index in range(1, total - 1):
            currentStoredWater = min(maxTillNowFromFront[index], maxTillNowFromBack[index]) - height[index]
            waterStored += currentStoredWater if currentStoredWater > 0 else 0
